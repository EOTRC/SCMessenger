...
### CTO DISPOSITION -- 2026-08-17. All six ruled. Section 2 is CLOSED.

Criterion applied, stated so the reasoning can be checked rather than trusted:
**D4 is a Pixel 6a exchanging a message with the AWS EC2 node, across networks.**
Two endpoints. The node has a **public IP and is not behind NAT**; the phone
dials **outbound**. No Windows desktop, no iOS, no BLE, no growing mesh.
Scoring is receiver decrypt + durable history + receipt.

A ticket is D4 work only if its trigger exists in **that** topology.
...
(a) dedicate ports per transport, (b) bind one transport per port and advertise
only what bound, (c) combined transport. AGENTS.md rule 9 sends architecture to
the operator, and this is architecture. **CTO recommendation: (b)** -- smallest
blast radius, removes the false advertisement rather than changing the port
model, and satisfies the ticket's own closing evidence ("netstat shows exactly
ONE listening socket per (address, port)"). Operator ruling required before
implementation.

**Two corrections to `HANDOFF/CEO_RULINGS_2026-08-16.md` §7.1, made while
verifying it.** It lists three missing manifest registrations. There were
**four** missing declarations, and `ShareReceiver` was not among them -- it was
never registered before `ebf5411b` either. The one the note missed was the
`VIEW` filter carrying the `scmessenger://` scheme, which is the
highest-severity of the four. Also, APK sharing *out* was never broken:
`ApkShareManager.kt:66` builds `ACTION_SEND` outbound, needing no filter. Only
inbound share was dead. See PR #176.

And a correction the CTO owes on his own earlier statement: the deep link is
