...
## [WARNING] CRITICAL: READ THIS FIRST
...
## [CRITICAL] URGENT: CRITICAL ISSUES DISCOVERED 2026-03-19

**Log audit reveals severe reliability issues:** [docs/historical/audits/LOG_AUDIT_REPORT_2026-03-19.md](../historical/audits/LOG_AUDIT_REPORT_2026-03-19.md)
...
## [TARGET] Standard Log Extraction Methods

### iOS Platform

**APPROVED METHOD:**
```bash
python3 scripts/ios_extractor.py
```

**Script Location:** `/ios_extractor.py` (repository root)

**What It Does:**
- [OK] Auto-detects connected iOS device via CoreDevice
- [OK] Extracts diagnostic snapshot from app container
- [OK] Runs continuous monitoring until Ctrl+C
- [OK] Two modes: Real-time streaming (idevicesyslog) or intelligent polling
- [OK] Graceful cleanup on exit
- [OK] Repository-specific configuration (no hardcoded paths)
...
**APPROVED METHOD:**
```bash
python3 scripts/adb_extractor.py
```

**Script Location:** `/adb_extractor.py` (repository root)

**What It Does:**
- [OK] Auto-detects connected Android device via ADB
- [OK] Extracts diagnostic snapshots from app private storage
- [OK] Runs continuous logcat monitoring until Ctrl+C
- [OK] PID-filtered for app-specific logs
- [OK] Graceful cleanup on exit
- [OK] Repository-specific configuration (no hardcoded paths)
...
**DO:**
1. [OK] Use `scripts/ios_extractor.py` for iOS log extraction
2. [OK] Use `scripts/adb_extractor.py` for Android log extraction
3. [OK] Reference these scripts when asking users for logs
4. [OK] Review captured logs from the standard output files
5. [OK] Mention the scripts in issue reports and debugging sessions

**DO NOT:**
1. [FAIL] Create ad-hoc log extraction commands
2. [FAIL] Ask users to manually run `adb logcat` or `idevicesyslog`
3. [FAIL] Use generic logging approaches without consulting these scripts
4. [FAIL] Hardcode device paths or bundle identifiers
5. [FAIL] Bypass the standardized extraction process
...
- [OK] "Logs captured using scripts/ios_extractor.py v1.0"
- [OK] "Android logs from scripts/adb_extractor.py (464 lines captured)"
- [FAIL] "Here are some logs I grabbed with adb logcat"

---

## [REVIEW] Why These Scripts Are Mandatory
...
### Both Platforms
- [OK] Scripts run continuously until Ctrl+C
- [OK] Safe to interrupt at any time
- [OK] Automatic cleanup on exit
- [OK] Works with device already connected and app running
...
2. Check `docs/historical/iOS/iOS_LOG_EXTRACTION_SUMMARY.md` for technical details
...
### Android (scripts/adb_extractor.py)
...
**Safe to capture:**
- [OK] Message IDs (UUIDs, no content)
- [OK] Peer IDs (libp2p identifiers)
- [OK] Transport metadata (BLE, relay, mDNS)
- [OK] Delivery states and timestamps
- [OK] Connection events and routing decisions

**NOT captured:**
- [FAIL] Message content/bodies
- [FAIL] User identities or contact names
- [FAIL] Private keys or secrets
- [FAIL] Personal information
...
- **iOS Technical Details:** `docs/historical/iOS/iOS_LOG_EXTRACTION_SUMMARY.md`
...
1. **iOS logs:** Use `scripts/ios_extractor.py` (mandatory)
2. **Android logs:** Use `scripts/adb_extractor.py` (mandatory)
...
**Status:** [OK] ACTIVE AND MANDATORY
