...
**Need help?** Run `python3 scripts/ios_extractor.py -h` for detailed usage information.

The script will:
1. [OK] Auto-detect your connected iOS device
2. [OK] Extract the diagnostic snapshot (most useful logs)
3. [OK] Start continuous monitoring (runs until Ctrl+C)
4. [OK] Clean up automatically when stopped
...
```bash
python3 scripts/ios_extractor.py       # Start log extraction
python3 scripts/ios_extractor.py -h    # Show detailed help
...
- **`docs/historical/iOS/iOS_LOG_EXTRACTION_SUMMARY.md`** ← Full technical details
...
The script runs continuously like Android's `scripts/adb_extractor.py`:
...
1. Start the extractor: `python3 scripts/ios_extractor.py`
...
**"Permission denied"**
- Make sure script is executable: `chmod +x scripts/ios_extractor.py`
- Or run with: `python3 scripts/ios_extractor.py`
...
**Need help?** Check `docs/historical/iOS/iOS_LOG_EXTRACTION_SUMMARY.md` for full technical details.
