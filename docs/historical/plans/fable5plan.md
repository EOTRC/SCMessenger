...
- **Verification:** `test -f README.md LICENSE CHANGELOG.md docs/ARCHITECTURE_MODULE_MAP.md`; every path referenced in docs/ARCHITECTURE_MODULE_MAP.md exists (scriptable link-check).
...
- **Verification:** Rust unit tests: given (battery, link set, payload size) → deterministic transport pick matching a documented decision table in `docs/ARCHITECTURE_MODULE_MAP.md`; grep proves Swift/Kotlin routers no longer contain independent preference ordering (only availability checks).
...
- **Verification:** `cargo test --workspace --all-features` and `cargo build --target wasm32-unknown-unknown` both green; `grep -rn "phase2_apis" core/` only in CHANGELOG; one authoritative routing-reputation source asserted in docs/ARCHITECTURE_MODULE_MAP.md.
