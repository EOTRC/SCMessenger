...
The function 'load_device_id' is defined in 'core\src\identity\store.rs' but is NEVER called anywhere in the repository. Your job is to fully integrate it into the overarching application loop. It may need to be wired into 1, 2, or 3+ different locations.

You MUST follow this strict analytical loop. Do not skip phases.

PHASE 1: CONTEXT GATHERING (Search & Ponder)
1. Open 'core\src\identity\store.rs' and read the implementation of 'load_device_id'. Understand its parameters, return type, and exact purpose.
...
3. Identify EVERY valid location in the codebase where 'load_device_id' MUST be invoked to work fully. 
...
--- CLOSEOUT EVIDENCE ---
VERIFIED WIRED: Called at core/src/identity/store.rs:128 (internal). Production call path confirmed.
