...
if ! docker ps | grep -q "scm-relay"; then
    echo -e "${RED}Error: Containers not running. Run ./scripts/verify_simulation.sh first${NC}"
    exit 1
fi
...
if [ -z "$BOB_ID" ] || [ -z "$ALICE_ID" ]; then
    echo -e "${RED}[FAIL] Failed to retrieve node IDs. Ensure containers are running.${NC}"
    exit 1
fi

echo -e "${GREEN}[OK] Node IDs retrieved${NC}"
echo "  Alice: $ALICE_ID"
echo "  Bob: $BOB_ID"
echo "  Relay: $RELAY_ID"
echo ""
...
echo -e "${GREEN}[OK] Alice peers: $INITIAL_PEERS_ALICE${NC}"
echo -e "${GREEN}[OK] Bob peers: $INITIAL_PEERS_BOB${NC}"
...
echo -e "${GREEN}[OK] Message queued in outbox during partition${NC}"
...
if [ "$RECOVERY_PEERS_ALICE" -gt 0 ] && [ "$RECOVERY_PEERS_BOB" -gt 0 ]; then
    echo -e "${GREEN}[OK] Network recovered successfully${NC}"
    ...
    echo -e "${RED}[FAIL] Network recovery failed${NC}"
...
if echo "$BOB_LOGS" | grep -q "$MESSAGE_PARTITION"; then
    echo -e "${GREEN}[OK] Queued message delivered after recovery${NC}"
    record_test "Partition Message Delivery" "pass"
else
    echo -e "${YELLOW}[WARNING] Queued message delivery pending or failed${NC}"
...
if [ "$ALICE_OBS" -gt 0 ] || [ "$BOB_OBS" -gt 0 ]; then
    echo -e "${GREEN}[OK] Address observation protocol working${NC}"
    record_test "Address Observation" "pass"
else
    echo -e "${YELLOW}[WARNING] Limited address observations (symmetric NAT?)${NC}"
...
if [ ! -z "$ALICE_CONNS" ]; then
    echo -e "${GREEN}[OK] Alice active connections:${NC}"
    echo "$ALICE_CONNS" | sed 's/^/     /'
fi

if [ ! -z "$BOB_CONNS" ]; then
    echo -e "${GREEN}[OK] Bob active connections:${NC}"
...
if [ "$RELAY_FORWARDS" -gt 0 ]; then
    echo -e "${GREEN}[OK] Circuit relay is actively forwarding${NC}"
...
if echo "$BOB_RELAY_LOG" | grep -q "$RELAY_TEST_MSG"; then
    echo -e "${GREEN}[OK] Message successfully relayed${NC}"
    ...
    echo -e "${RED}[FAIL] Message delivery failed${NC}"
...
if [ "$ROUTING_UPDATES" -gt 0 ]; then
    echo -e "${GREEN}[OK] Mesh routing tables are being updated${NC}"
else
    echo -e "${YELLOW}[WARNING] Limited routing activity (static topology?)${NC}"
fi
...
if [ "$PEER_EXCHANGE_ALICE" -gt 0 ] || [ "$PEER_EXCHANGE_BOB" -gt 0 ]; then
    echo -e "${GREEN}[OK] Peer exchange protocol active${NC}"
fi
...
if [ "$TCP_USAGE" -gt 0 ]; then
    echo -e "${GREEN}[OK] TCP transport active${NC}"
    TRANSPORT_ACTIVE=true
fi
if [ "$QUIC_USAGE" -gt 0 ]; then
    echo -e "${GREEN}[OK] QUIC transport active${NC}"
    TRANSPORT_ACTIVE=true
fi
if [ "$WS_USAGE" -gt 0 ]; then
    echo -e "${GREEN}[OK] WebSocket transport active${NC}"
    TRANSPORT_ACTIVE=true
fi
...
if [ "$TRANSPORT_ACTIVE" = true ]; then
    ...
    echo -e "${RED}[FAIL] No transport protocol activity detected${NC}"
...
if [ "$ESCALATION" -gt 0 ]; then
    echo -e "${GREEN}[OK] Transport escalation detected: $ESCALATION events${NC}"
    record_test "Transport Escalation" "pass"
else
    echo -e "${YELLOW}[WARNING] No transport escalation detected${NC}"
...
if [ "$CIRCUITS" -gt 0 ]; then
    echo -e "${GREEN}[OK] Onion routing circuits established${NC}"
else
...
if [ "$COVER" -gt 0 ]; then
    echo -e "${GREEN}[OK] Cover traffic active: $COVER events${NC}"
else
...
echo -e "${GREEN}[OK] Message queued while Bob offline${NC}"
...
if echo "$BOB_OFFLINE_LOG" | grep -q "$OFFLINE_MSG"; then
    echo -e "${GREEN}[OK] Offline message delivered via Drift sync${NC}"
else
    echo -e "${YELLOW}[WARNING] Message delivery pending (may take time)${NC}"
fi
...
if [ "$STORED" -gt 0 ]; then
    echo -e "${GREEN}[OK] Relay is storing and forwarding: $STORED events${NC}"
fi
...
echo -e "${GREEN}[OK] Sent 10 messages in ${DURATION}s${NC}"
...
if [ "$TOTAL_ERRORS" -eq 0 ]; then
    ...
    echo -e "${YELLOW}[WARNING] Minor errors detected (< 5)${NC}"
else
    echo -e "${RED}[FAIL] Multiple errors detected${NC}"
fi
...
if [ "$FAILED_TESTS" -eq 0 ]; then
    echo -e "${GREEN}[OK] Comprehensive Network Testing Complete${NC}"
else
    echo -e "${YELLOW}[WARNING] Network Testing Complete with Issues${NC}"
fi
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
echo -e "${BLUE}Test Statistics:${NC}"
echo "  Total Tests:    $TOTAL_TESTS"
echo -e "  ${GREEN}[OK] Passed:       $PASSED_TESTS${NC}"
echo -e "  ${YELLOW}[WARNING] Warnings:     $WARNING_TESTS${NC}"
echo -e "  ${RED}[FAIL] Failed:       $FAILED_TESTS${NC}"
echo ""
...
if [ "$FAILED_TESTS" -gt 0 ]; then
    echo -e "${RED}[WARNING] ATTENTION: $FAILED_TESTS test(s) failed!${NC}"
    echo ""
