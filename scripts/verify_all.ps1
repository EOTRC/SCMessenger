...
$LogFile = "$RepoRoot\SCMessenger_Diagnostics.log"
$script:Summary = @()
...
function Run-Gate ($TaskName, $Command, $WorkingDirectory = $RepoRoot) {
    ...
    $process = Start-Process -FilePath "cmd.exe" -ArgumentList "/c $Command" -Wait -NoNewWindow -PassThru -RedirectStandardOutput "$RepoRoot\temp_out.txt" -RedirectStandardError "$RepoRoot\temp_err.txt"
    
    $exitCode = $process.ExitCode
    $out = Get-Content "$RepoRoot\temp_out.txt" -ErrorAction SilentlyContinue
    $err = Get-Content "$RepoRoot\temp_err.txt" -ErrorAction SilentlyContinue
    ...
    # Cleanup temp files
    Remove-Item "$RepoRoot\temp_out.txt", "$RepoRoot\temp_err.txt" -ErrorAction SilentlyContinue
