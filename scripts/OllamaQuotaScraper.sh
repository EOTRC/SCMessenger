...
# Locate repo: this script lives in <repo>/scripts/, so the root is one level up.
...
# Session cookie is read from the OLLAMA_SESSION_COOKIE environment variable.
COOKIE="${OLLAMA_SESSION_COOKIE:-}"
if [ -z "$COOKIE" ]; then
    echo "[ERROR] OLLAMA_SESSION_COOKIE is not set. Export your ollama.com session cookie before running." >&2
    exit 1
fi
