...
foreach ($Task in $Tasks) {
    ...
    if ($LogOutput -match "STATUS: SUCCESS_STOP") {
        Write-Host "[OK] Task Successful! Moving to done..." -ForegroundColor Green
        Move-Item -Path $Task.FullName -Destination $DoneDir -Force
    }
    elseif ($LogOutput -match "STATUS: ESCALATE_TO_ORCHESTRATOR") {
        Write-Host "[WARNING] Task Failed repeatedly. Escalating for 671B model..." -ForegroundColor Yellow
        Move-Item -Path $Task.FullName -Destination $BacklogDir -Force
    }
    else {
        Write-Host "[FAIL] Unexpected exit or crash. Leaving in todo\ for next retry." -ForegroundColor Red
    }
