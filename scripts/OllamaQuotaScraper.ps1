...
$cookie = $env:OLLAMA_SESSION_COOKIE
if (-not $cookie) {
    Write-Host "[ERROR] OLLAMA_SESSION_COOKIE is not set. Export your ollama.com session cookie before running." -ForegroundColor Red
    exit 1
}

# This script lives in <repo>\scripts\, so the repo root is one level up.
$baseDir     = if ($env:SCM_REPO_ROOT) { $env:SCM_REPO_ROOT } else { Split-Path -Parent $PSScriptRoot }
