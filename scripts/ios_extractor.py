...
DOCUMENTATION:
    Quick Start:     docs/ops/QUICKSTART_IOS_LOGS.md
    Full Details:    docs/historical/iOS/iOS_LOG_EXTRACTION_SUMMARY.md
...
def main():
    ...
    try:
        ...
        if stream_handle:
            ...
            print_success("\n[OK] Log extraction setup completed successfully")
    ...
    if os.path.exists(LIVE_LOG_OUTPUT):
        size = os.path.getsize(LIVE_LOG_OUTPUT)
        results.append(f"[OK] Live logs: {LIVE_LOG_OUTPUT} ({size} bytes)")
    else:
        results.append(f"[FAIL] Live logs: FAILED")
    
    if os.path.exists(SNAPSHOT_OUTPUT):
        size = os.path.getsize(SNAPSHOT_OUTPUT)
        results.append(f"[OK] Diagnostic snapshot: {SNAPSHOT_OUTPUT} ({size} bytes)")
    else:
        results.append(f"[FAIL] Diagnostic snapshot: FAILED")
