...
echo -e "${GREEN}[OK] Network simulation started${NC}"
echo ""

echo -e "${BLUE}Step 4: Waiting for network initialization (20s)...${NC}"
sleep 20
echo -e "${GREEN}[OK] Network initialized${NC}"
echo ""
...
if docker ps | grep -q "scm-nat-a" && docker ps | grep -q "scm-nat-b"; then
    echo -e "${GREEN}[OK] NAT gateways running${NC}"
else
    echo -e "${RED}[FAIL] NAT gateways not running${NC}"
fi
...
if [ "$RELAY_RUNNING" -gt 0 ] && [ "$ALICE_RUNNING" -gt 0 ] && [ "$BOB_RUNNING" -gt 0 ]; then
    echo -e "${GREEN}[OK] All nodes running (Relay, Alice, Bob)${NC}"
else
    echo -e "${RED}[FAIL] Some nodes failed to start${NC}"
...
if [ "$ALICE_TC" -gt 0 ]; then
    echo -e "${GREEN}[OK] Alice traffic control active${NC}"
    docker exec scm-alice tc qdisc show | head -5
else
    echo -e "${YELLOW}[WARNING] Alice traffic control not detected${NC}"
fi

if [ "$BOB_TC" -gt 0 ]; then
    echo -e "${GREEN}[OK] Bob traffic control active${NC}"
    docker exec scm-bob tc qdisc show | head -5
else
    echo -e "${YELLOW}[WARNING] Bob traffic control not detected${NC}"
fi
...
if [ -z "$BOB_ID" ] || [ -z "$ALICE_ID" ] || [ -z "$RELAY_ID" ]; then
    echo -e "${RED}[FAIL] Failed to retrieve node IDs${NC}"
    echo "Alice: $ALICE_ID"
    echo "Bob: $BOB_ID"
    echo "Relay: $RELAY_ID"
    exit 1
...
if echo "$BOB_LOGS" | grep -q "$TEST_MSG"; then
    ...
    echo -e "${YELLOW}[WARNING] Message not yet delivered (may need more time)${NC}"
fi
...
echo -e "  ${YELLOW}./scripts/test_network_scenarios.sh${NC}"
echo ""
echo "View live logs:"
