...
class LogExtractor:
    ...
    def cleanup(self):
        ...
        if self.logcat_process and self.logcat_process.poll() is None:
            ...
            try:
                ...
                print("[Cleanup] [OK] Logcat process terminated cleanly")
            except subprocess.TimeoutExpired:
                print("[Cleanup] [WARN] SIGTERM timeout, sending SIGKILL...")
                self.logcat_process.kill()
                self.logcat_process.wait()
                print("[Cleanup] [OK] Logcat process killed")
    ...
    def cleanup_old_logs(self):
        ...
        if total_size > max_size_bytes:
            ...
            print(f"[Cleanup] [OK] Log cleanup complete. Total size now: {total_size / 1024 / 1024:.1f} MB")
    ...
    def verify_device_connected(self):
        ...
        if not devices or "unauthorized" in result.stdout:
            print("[ERROR] [FAIL] No authorized ADB device found")
            return False
            
        print(f"[Verification] [OK] Device connected: {devices[0]}")
        return True
    ...
    def verify_app_running(self):
        ...
        if result.returncode != 0 or not result.stdout.strip():
            print(f"[WARNING] [WARN] App may not be running (no PID found)")
            return False
            
        pid = result.stdout.strip()
        print(f"[Verification] [OK] App is running (PID: {pid})")
        return True
    ...
    def start_diagnostic_polling(self):
        ...
        try:
            ...
            if result.returncode == 0 and result.stdout:
                ...
                print_success(f"[Polling] [OK] Initial snapshot captured: {line_count} lines")
                print_info("[Polling] These are app-generated diagnostic events (delivery attempts, BLE rx/tx, etc.)")
            else:
                print_info("[Polling] [WARN] No diagnostic log found yet (app may need to generate events)")
                
        except Exception as e:
            print_info(f"[Polling] [WARN] Initial snapshot failed: {e}")
        
        print_success(f"[Polling] [OK] Polling mode initialized: {LIVE_LOGCAT_FILE}")
    ...
    def trigger_app_activity(self):
        ...
        if result.returncode == 0:
            print("[Trigger] [OK] Brought app to foreground")
    ...
    def verify_live_stream(self, logcat_file):
        ...
        while time.time() - verification_start < 5.0:
            ...
            if self.logcat_process.poll() is not None:
                print(f"[ERROR] [FAIL] Logcat process died (exit code: {self.logcat_process.returncode})")
        ...
        if app_lines_captured > 0:
            print(f"[Verification] [OK] SUCCESS: Live stream is capturing app logs")
            return True
        elif lines_captured > 0:
            print(f"[Verification] [WARN] WARNING: Capturing logs but no app-specific content yet")
            print(f"[Verification]   This may be normal if app is idle")
            return True
        else:
            print(f"[Verification] [WARN] WARNING: App appears idle, no new logs generated")
    ...
    def pull_diagnostic_snapshots(self):
        ...
        for log_path in logs_to_pull:
            ...
            if result.returncode == 0 and len(result.stdout) > 0:
                ...
                print(f"[Pull] [OK] SUCCESS: {filename} ({size} bytes)")
                pulled_files.append((filename, local_path, size))
            else:
                print(f"[Pull] [WARN] SKIP: {filename} (not found or empty)")
    ...
    def verify_snapshots(self, pulled_files):
        ...
        if not pulled_files:
            print("[Verification] [FAIL] FAILURE: No diagnostic files pulled")
            return False
        ...
        if total_size > 0:
            print("[Verification] [OK] SUCCESS: Diagnostic snapshots extracted")
            return True
        else:
            print("[Verification] [FAIL] FAILURE: All files empty")
            return False
        ...
        try:
            while True:
                ...
                try:
                    ...
                    print_error(f"[Polling] [FAIL] Poll #{snapshot_count}: Timeout pulling snapshot")
                except Exception as e:
                    print_error(f"[Polling] [FAIL] Poll #{snapshot_count}: Error - {e}")
    ...
    def generate_summary(self, pulled_files):
        ...
        if os.path.exists(LIVE_LOGCAT_FILE):
            ...
            print(f"  Status: {'[OK] Active (still streaming)' if self.logcat_process and self.logcat_process.poll() is None else '[OK] Completed'}")
...
USAGE EXAMPLES:
    # Standard extraction (most common), run from the repo root
...
DOCUMENTATION:
    Standard:        docs/ops/LOG_EXTRACTION_STANDARD.md
    Rotation:        docs/ops/LOG_ROTATION_INFO.md
    Quick Ref:       docs/ops/LOG_EXTRACTION_QUICK_REF.md
...
For more help, see: docs/ops/LOG_EXTRACTION_STANDARD.md
""".format(
...
def main():
    ...
    try:
        ...
        if args.logcat:
            ...
            if not QUIET_MODE:
                print("\n[OK] Initial polling setup completed")
                print(f"[OK] Polling results being written to: {LIVE_LOGCAT_FILE}")
        ...
        if not QUIET_MODE:
            ...
            if os.path.exists(LIVE_LOGCAT_FILE):
                size = os.path.getsize(LIVE_LOGCAT_FILE)
                print(f"  [OK] Live logs: {LIVE_LOGCAT_FILE} ({size} bytes)")
            ...
            if os.path.exists(DIAGNOSTIC_SNAPSHOT_DIR):
                ...
                if files:
                    ...
                    print(f"  [OK] Diagnostic snapshots: {DIAGNOSTIC_SNAPSHOT_DIR}/ ({len(files)} files, {total_size} bytes)")
