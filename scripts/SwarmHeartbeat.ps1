...
function Invoke-QuotaRefresh {
    ...
    $scraperPath = Join-Path $Script:ScriptDir "OllamaQuotaScraper.ps1"
...
function Write-EfficiencyLedger {
    $ledgerFile = Join-Path $Script:BaseDir "docs\orchestration\API_EFFICIENCY_LEDGER.md"
...
function Invoke-DispatchWorker {
    ...
    try {
        ...
        $job = Start-Job -Name $jobName -ArgumentList $inProgressPath, $model, $budget, $Script:BaseDir, $quotaContext, $Script:ScriptDir -ScriptBlock {
            param($TaskFile, $Model, $BudgetLimit, $BaseDir, $QuotaContext, $ScriptDir)
            Set-Location $BaseDir
            $qcJson = ($QuotaContext | ConvertTo-Json -Compress)
            & "$ScriptDir\TaskGovernor.ps1" -TaskFile $TaskFile -Model $Model -BudgetLimit $BudgetLimit -QuotaContextJson $qcJson
        }
